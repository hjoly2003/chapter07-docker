...
class GameServiceImpl implements GameService {
    ...
    /**
     * Called from {@code GameEventHandler#handleMultiplicationSolved}.<p/>
     * The {@code @Transactional} annotation ensures that if we have a problem, DB operations will be rolled back,the message will be rejected by default and we can requeue it if that’s what we want.
     */
    @Override
    @Transactional
    public GameResult newAttemptForUser(final ChallengeSolvedEvent challenge) {
        // We give points only if it's correct
        // [N]: The event is from the multiplicaton service's "attempt.correct" routing key. Hence, this challenge is implicitly correct. But we keep this test for readability and for not beeing too dependant on the routing logic.
